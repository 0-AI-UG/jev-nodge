...
/// Appends a line to ~/Library/Logs/Nodge.log.
func log(_ message: String) {
    let url = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/Nodge.log")
...
enum Jev {
    ...
    static func choose(state: [String: Any], instructions: String, criteria: [(String, String)]) async throws -> Decision {
        guard let key = await AppSettings.shared.openRouterKey, !key.isEmpty else {
            throw JevError(errorDescription: "OpenRouter is not configured")
        }
        let model = await AppSettings.shared.jevModel
...
enum Parse {
    ...
    static func url(from heard: String) -> URL? {
        ...
        let site = siteName(s)
        guard !site.isEmpty, !site.contains(" "),
              site.range(of: #"^[a-z0-9-]+$"#, options: .regularExpression) != nil else { return nil }
