...
enum Step: Decodable {
    ...
    static let builtins: Set<String> = [
        "open_app", "open_url", "search", "type_text", "click", "quit_app", "repeat", "emoji", "set_volume", "open_folder", "screenshot", "ask_ai", "computer_task",
    ]
...
enum Registry {
    /// ~/.config/nodge/config.json, or $NODGE_CONFIG.
    static let userFile = env["NODGE_CONFIG"].map { URL(fileURLWithPath: NSString(string: $0).expandingTildeInPath) }
        ?? FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".config/nodge/config.json")
