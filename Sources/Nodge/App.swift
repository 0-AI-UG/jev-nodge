...
final class Controller: NSObject {
    ...
    func start() {
        ...
        dumpSignal.setEventHandler {
            ...
            Task.detached {
                ...
                let url = FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/Nodge-ui.log")
    ...
    @objc private func openLog() {
        NSWorkspace.shared.open(FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/Nodge.log"))
    }
    ...
    /// Only wake-phrase utterances and short follow-ups reach Jev. The microphone stays local before this gate.
    ...
    /// A bounded native-app loop. Jev chooses only from controls observed on the current screen and
    /// a small fixed action set. A generative model may supply text, but never selectors or commands.
    private func computerTask(goal: String, initialProbs: [Double]) async throws {
        var history: [String] = []
        ...
        for step in 1...12 {
            ...
            if typedText == nil {
                typedText = try await AIClient.textPayload(for: goal, frontmostApp: app.localizedName ?? "")
            }

            var criteria: [(String, String)] = labels.prefix(70).enumerated().map {
                ("click_\($0.offset)", "click the visible control labeled ‘\($0.element)’")
            }
            if let typedText, !typedText.isEmpty {
                criteria.append(("type_text", "type the task's requested text into the focused or visible text field"))
            }
            criteria += [
                ("scroll_down", "scroll down to reveal later content"),
                ("scroll_up", "scroll up to reveal earlier content"),
                ("press_enter", "press Enter to submit or confirm"),
                ("press_escape", "press Escape to close or cancel a transient overlay"),
                ("done", "the requested task is visibly complete"),
                ("blocked", "the task cannot safely continue from the visible interface"),
            ]
            let choice = try await Jev.choose(
                state: [
                    "goal": goal,
                    "frontmost_app": app.localizedName ?? "",
                    "visible_controls": Array(labels.prefix(70)),
                    "actions_taken": history,
                ],
                instructions: "Choose the single safest next action for this macOS task. Click only an observed control. Choose done only when the visible state demonstrates completion. Choose blocked instead of guessing.",
                criteria: criteria
            )
            guard choice.confidence >= 0.35 else {
                hud.show(.card, title: "Needs help", subtitle: "I’m not confident about the next step.", hint: "Stopped after \(history.count) actions", probs: choice.probs)
                return
            }
            ...
            case "blocked":
                ...
                return
            case "type_text":
                guard let typedText, !typedText.isEmpty else { return }
                await Task.detached {
                    Actions.focusTextField(pid: pid)
                    Actions.type(typedText)
                }.value
                history.append("typed requested text")
            case "scroll_down":
                await Task.detached { Actions.press(121) }.value
                history.append("scrolled down")
            case "scroll_up":
                await Task.detached { Actions.press(116) }.value
                history.append("scrolled up")
            case "press_enter":
                await Task.detached { Actions.press(36) }.value
                history.append("pressed Enter")
            case "press_escape":
                await Task.detached { Actions.press(53) }.value
                history.append("pressed Escape")
            default:
                guard choice.choice.hasPrefix("click_"),
                      let index = Int(choice.choice.dropFirst(6)),
                      labels.indices.contains(index) else {
                    hud.show(.card, title: "Stopped safely", subtitle: "Jev returned an invalid action.", probs: choice.probs)
                    return
                }
                let label = labels[index]
                hud.show(.card, title: "Working", subtitle: "Click \(label)", hint: "Step \(step) of 12 · \(app.localizedName ?? "Mac")", probs: choice.probs)
                await Task.detached { Actions.click(label: label, among: targets) }.value
                history.append("clicked \(label)")
            }
            try await Task.sleep(nanoseconds: 450_000_000)
        }
        hud.show(.card, title: "Stopped safely", subtitle: "The task reached its 12-action limit.", hint: "No further actions were taken", probs: initialProbs)
    }
    ...
    func demo() {
        ...
        for (at, step) in steps {
            DispatchQueue.main.asyncAfter(deadline: .now() + at) { step() }
        }
    }
}
...
struct Main {
    static func main() async {
        ...
        if CommandLine.arguments.contains("--demo") { controller.demo() } else { controller.launch() }
